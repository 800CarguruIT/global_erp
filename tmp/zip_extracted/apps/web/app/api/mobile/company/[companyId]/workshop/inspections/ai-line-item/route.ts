...
import { handleMobileError } from "@/app/api/mobile/utils";
...
const fallbackQuestions: LineItemAiQuestion[] = [
  {
    id: "q1",
    text: "Is there visible wear or damage on this part?",
    critical: true,
  },
  {
    id: "q2",
    text: "Does this issue affect safe driving right now?",
    critical: true,
  },
  {
    id: "q3",
    text: "Can this be monitored until next service?",
    critical: false,
  },
];
...
export async function POST(req: NextRequest, { params }: Params) {
  try {
    ...
    const answers =
      ((body?.answers ?? null) as Record<
        string,
        LineItemAiAnswerValue
      > | null) ?? null;
    ...
    const allowed = await canUseAi("ai.workshop.inspection.line_item" as any, {
      companyId,
    }).catch(() => true);
    ...
    if (!allowed || !resolved.client) {
      return NextResponse.json({
        questions:
          existingQuestions.length > 0 ? existingQuestions : fallbackQuestions,
        recommendation:
...
- Keep question text very simple and clear.
- This is one-step question generation. Do not create follow-up questions.
...
`;
    ...
    console.error(
      "POST /api/mobile/company/[companyId]/workshop/inspections/ai-line-item error:",
      error,
    );
