...
export async function GET(req: NextRequest, { params }: Params) {
  try {
    ...
    const priority = String(searchParams.get("priority") ?? "")
      .trim()
      .toLowerCase();
    const bookingType = String(searchParams.get("bookingType") ?? "")
      .trim()
      .toLowerCase();
    ...
    return createMobileSuccessResponse({ bookings: filtered });
  } catch (error) {
    console.error(
      "GET /api/mobile/company/[companyId]/sales/leads/bookings error:",
      error,
    );
