  ...
  try {
    ...
    const statusFilter = (() => {
      ...
      if (normalized === "quoted") {
        return sql`AND (
          LOWER(COALESCE(pq.status, '')) IN ('pending', 'quoted', 'approved')
          OR (
            LOWER(COALESCE(pq.status, '')) = 'pending'
            AND LOWER(COALESCE(li.status, iori.status, '')) IN ('approved', 'inquiry', 'pending')
          )
        )`;
      }
      if (normalized === "ordered") {
        return sql`AND LOWER(COALESCE(pq.status, '')) = 'ordered'`;
      }
      if (normalized === "rejected") {
        return sql`AND LOWER(COALESCE(pq.status, '')) = 'rejected'`;
      }
      ...
      if (normalized === "returns") {
        return sql`AND LOWER(COALESCE(pq.status, '')) IN ('return', 'returned', 'returns')`;
      }
    ...
    const searchFilter = q
      ? sql`AND (
          LOWER(COALESCE(v.name, '')) LIKE ${`%${q}%`}
          OR LOWER(COALESCE(li.product_name, iori.part_name, '')) LIKE ${`%${q}%`}
          OR LOWER(COALESCE(car.plate_number, '')) LIKE ${`%${q}%`}
          OR LOWER(COALESCE(ior.request_number, '')) LIKE ${`%${q}%`}
        )`
      : sql``;
    ...
    console.error(
      "GET /api/mobile/company/[companyId]/part-quotes error:",
      error,
    );
