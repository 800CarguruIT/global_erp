...
export async function GET(req: NextRequest, { params }: Params) {
  try {
    ...
    const bids = rows.map((row: any) => ({
      ...
      remarks: row.remarks,
      status:
        normalizedStatusParam === "ordered" &&
        String(row.line_item_status ?? "").toLowerCase() === "approved"
          ? "Approved"
          : row.status,
      updatedAt: row.updated_at,
    }));

    return createMobileSuccessResponse({ bids });
  } catch (error) {
    console.error(
      "GET /api/mobile/company/[companyId]/vendors/[vendorId]/bids error:",
      error,
    );
