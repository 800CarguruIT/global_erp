...
-- Populate insurance_name from insurance_data by matching on phone (skip if table doesn't exist)
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'insurance_data') THEN
    UPDATE customers c
