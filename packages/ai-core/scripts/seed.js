...
async function main() {
  ...
  `;
    for (const b of branches) {
        if (b.code === "BR-DWTN")
            branchDowntownId = b.id;
        if (b.code === "BR-IND")
            branchIndustrialId = b.id;
    }
  ...
  `;
    if (!globalRole) {
        throw new Error("global_admin role not found. Run migrations before seeding.");
    }
    await sql `
  ...
  `;
    for (const e of employees) {
        if (e.auto_code === "EMP-0001")
            empManagerId = e.id;
        if (e.auto_code === "EMP-0002")
            empTechId = e.id;
        if (e.auto_code === "EMP-0003")
            empAgentId = e.id;
    }
    console.log("Seeding branch users linked to employees...");
    const [branchManagerRole] = await sql `
    SELECT id FROM roles WHERE key = 'branch_manager' LIMIT 1
  `;
    const [branchTechRole] = await sql `
    SELECT id FROM roles WHERE key = 'branch_technician' LIMIT 1
  `;
    const [branchAgentRole] = await sql `
  ...
  `;
    ...
    for (const [idx, c] of calls.entries()) {
        const createdAt = new Date(now.getTime() - idx * 3600000);
        await sql `
