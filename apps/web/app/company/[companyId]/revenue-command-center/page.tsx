    ...
    <div className="mx-auto max-w-[1600px] space-y-5 py-4 px-4">
      ...
      <div className="grid grid-cols-5 gap-3">
        {(data.kpis as Kpi[]).map((k, i) => (
          <div key={i} className={`rounded-xl p-4 ${i === 4 ? "bg-red-500/[0.06]" : "bg-white/[0.03]"}`}>
        ...
        <div className="rounded-xl bg-white/[0.03] p-5">
          ...
          <div className="space-y-1 max-h-64 overflow-y-auto">
            {[...(data.leaderboard as LeaderboardEntry[])]
              ...
              .map((a, i) => (
                <div key={i} className="flex items-center gap-3 text-sm py-1.5 rounded-lg hover:bg-white/[0.03] px-1 -mx-1 transition-colors">
              ...
              {DAY_LABELS.map((day, di) => {
                ...
                return [
                  ...
                  ...Array.from({ length: 12 }, (_, hi) => {
                    ...
                    return (
                      <div key={`${di}-${hi}`}
                        className="rounded h-6 flex items-center justify-center text-[9px] text-white/70"
