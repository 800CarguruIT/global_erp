      ...
      <div className="space-y-4 py-4">
        <div className="flex flex-wrap items-start justify-between gap-4">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <Link href={companyId ? `/company/${companyId}/cars/new` : "#"} className="inline-flex items-center rounded-md border border-white/30 bg-primary px-4 py-2 text-[11px] font-semibold uppercase tracking-wide text-primary-foreground shadow-md transition hover:opacity-90 hover:shadow-lg">Add Car</Link>
          </div>
        </div>
              ...
              <table className="min-w-full text-sm border-separate border-spacing-0">
                <thead>
                  <tr className="text-left bg-slate-950">
                    <th className="px-4 py-3.5 sticky left-0 top-0 z-30 bg-slate-950 border-b border-slate-700/60 text-[11px] font-semibold uppercase tracking-wide text-slate-300 shadow-[inset_-1px_0_0_rgba(148,163,184,0.22)]"><input type="checkbox" checked={filtered.length > 0 && filtered.every((c) => selected[c.id])} onChange={(e) => { const next = { ...selected }; filtered.forEach((c) => { next[c.id] = e.target.checked; }); setSelected(next); }} /></th>
                    <th className="px-4 py-3.5 sticky top-0 z-20 bg-slate-950 border-b border-slate-700/60 text-[11px] font-semibold uppercase tracking-wide text-slate-300"><button type="button" onClick={() => applyColumnSort("plate_number")} className="inline-flex items-center gap-1.5 hover:text-white">Plate {sortLabel("plate_number") ? <span className="rounded bg-white/10 px-1.5 py-0.5 text-[9px] font-bold tracking-normal text-slate-200">{sortLabel("plate_number")}</span> : null}</button></th>
                    <th className="px-4 py-3.5 sticky top-0 z-20 bg-slate-950 border-b border-slate-700/60 text-[11px] font-semibold uppercase tracking-wide text-slate-300"><button type="button" onClick={() => applyColumnSort("vin")} className="inline-flex items-center gap-1.5 hover:text-white">VIN {sortLabel("vin") ? <span className="rounded bg-white/10 px-1.5 py-0.5 text-[9px] font-bold tracking-normal text-slate-200">{sortLabel("vin")}</span> : null}</button></th>
                    <th className="px-4 py-3.5 sticky top-0 z-20 bg-slate-950 border-b border-slate-700/60 text-[11px] font-semibold uppercase tracking-wide text-slate-300"><button type="button" onClick={() => applyColumnSort("make")} className="inline-flex items-center gap-1.5 hover:text-white">Make / Model {sortLabel("make") ? <span className="rounded bg-white/10 px-1.5 py-0.5 text-[9px] font-bold tracking-normal text-slate-200">{sortLabel("make")}</span> : null}</button></th>
                    <th className="px-4 py-3.5 sticky top-0 z-20 bg-slate-950 border-b border-slate-700/60 text-[11px] font-semibold uppercase tracking-wide text-slate-300"><button type="button" onClick={() => applyColumnSort("model_year")} className="inline-flex items-center gap-1.5 hover:text-white">Year {sortLabel("model_year") ? <span className="rounded bg-white/10 px-1.5 py-0.5 text-[9px] font-bold tracking-normal text-slate-200">{sortLabel("model_year")}</span> : null}</button></th>
                    <th className="px-4 py-3.5 sticky top-0 z-20 bg-slate-950 border-b border-slate-700/60 text-[11px] font-semibold uppercase tracking-wide text-slate-300"><button type="button" onClick={() => applyColumnSort("code")} className="inline-flex items-center gap-1.5 hover:text-white">Code {sortLabel("code") ? <span className="rounded bg-white/10 px-1.5 py-0.5 text-[9px] font-bold tracking-normal text-slate-200">{sortLabel("code")}</span> : null}</button></th>
                    <th className="px-4 py-3.5 sticky top-0 z-20 bg-slate-950 border-b border-slate-700/60 text-[11px] font-semibold uppercase tracking-wide text-slate-300">Status</th>
                    <th className="px-4 py-3.5 text-right sticky right-0 top-0 z-30 bg-slate-950 border-b border-slate-700/60 text-[11px] font-semibold uppercase tracking-wide text-slate-300 shadow-[inset_1px_0_0_rgba(148,163,184,0.22)]">Actions</th>
                  </tr>
                </thead>
                <tbody>
