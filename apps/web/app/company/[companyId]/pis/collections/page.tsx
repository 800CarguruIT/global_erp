  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <h2 className="text-lg font-bold text-white">Collections & Revenue</h2>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[{l:"Total Revenue",v:`AED ${Math.round(data.totalRevenue/1000)}K`,c:"text-white"},{l:"Gross Profit",v:`AED ${Math.round(data.grossProfit/1000)}K`,c:"text-emerald-400"},{l:"GP %",v:`${data.gpPct}%`,c:"text-yellow-400"},{l:"Collection Rate",v:`${data.collectionRate}%`,c:"text-cyan-400"},{l:"DSO",v:`${data.dso} days`,c:"text-white"},{l:"Overdue Amount",v:`AED ${Math.round(data.overdueAmount/1000)}K`,c:"text-red-400"},{l:"Overdue Count",v:data.overdueCount,c:"text-red-400"},{l:"Total Cost",v:`AED ${Math.round(data.totalCost/1000)}K`,c:"text-slate-300"}].map(k => (
          <div key={k.l} className="rounded-xl border border-white/10 bg-gradient-to-br from-slate-900/80 to-black/90 p-4">
      ...
      <div className="rounded-xl border border-white/10 bg-gradient-to-br from-slate-900/80 to-black/90 p-5">
        <h2 className="text-sm font-bold text-white mb-4">PER ADVISOR GP%</h2>
        <table className="w-full text-xs"><thead><tr className="border-b border-white/10 text-left">
          ...
          {(data.perAdvisor??[]).map((a:any)=>(
            <tr key={a.advisorName} className="border-b border-white/5">
              <td className="px-4 py-2 text-white font-medium">{a.advisorName}</td><td className="px-4 py-2 text-slate-300">AED {Math.round(a.revenue/1000)}K</td>
