  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-xl border border-white/10 bg-gradient-to-br from-slate-900/80 to-black/90 p-5">
        <h2 className="text-sm font-bold text-white mb-4">PER ADVISOR BREAKDOWN</h2>
        <table className="w-full text-xs"><thead><tr className="border-b border-white/10 text-left">
          ...
          {(data.perAdvisor??[]).map((a:any)=>(
            <tr key={a.advisorName} className="border-b border-white/5">
              <td className="px-4 py-2 text-white font-medium">{a.advisorName}</td><td className="px-4 py-2 text-slate-300">{a.total}</td>
              <td className="px-4 py-2 text-emerald-400">{a.approved}</td><td className="px-4 py-2 text-red-400">{a.rejected}</td>
              <td className="px-4 py-2 text-amber-400">{a.pending}</td><td className="px-4 py-2 text-white font-bold">{a.approvalRate}%</td>
            </tr>))}
        </tbody></table>
      </div>
    </div>
