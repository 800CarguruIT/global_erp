  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
        {[{l:"Active WIP",v:data.activeWip,c:"text-cyan-400"},{l:"Capacity",v:data.totalCapacity,c:"text-white"},{l:"On-Time %",v:`${data.onTimePct}%`,c:data.onTimePct>=80?"text-emerald-400":"text-red-400"},{l:"Pickup Rate",v:`${data.pickupRate}%`,c:"text-white"},{l:"Zero Revenue WIP",v:data.zeroRevenueWip,c:data.zeroRevenueWip>0?"text-red-400":"text-emerald-400"}].map(k => (
      ...
      <div className="rounded-xl border border-white/10 bg-gradient-to-br from-slate-900/80 to-black/90 p-5">
        <h2 className="text-sm font-bold text-white mb-4">PER ADVISOR WIP</h2>
        <table className="w-full text-xs"><thead><tr className="border-b border-white/10 text-left">
          ...
          {(data.perAdvisor??[]).map((a:any)=>(
            <tr key={a.advisorName} className="border-b border-white/5"><td className="px-4 py-2 text-white font-medium">{a.advisorName}</td><td className="px-4 py-2 text-slate-300">{a.activeJobs}</td><td className="px-4 py-2 text-slate-300">{a.onTimePct}%</td></tr>))}
        </tbody></table>
      </div>
    </div>
  );
