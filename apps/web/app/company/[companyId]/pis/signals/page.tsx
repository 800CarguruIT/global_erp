  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h2 className="text-lg font-bold text-foreground">Full Signal Library <span className="text-slate-500 font-normal ml-2">{signals.length} signals</span></h2>
        <div className="flex gap-2">
          {["ALL","HIGH","MED","LOW"].map(f => (
            <button key={f} onClick={() => setFilter(f)} className={`px-3 py-1.5 rounded text-[10px] font-bold ${filter===f?"bg-amber-500/20 text-amber-400":"bg-slate-800 text-slate-400 hover:text-foreground"}`}>{f}</button>
          ))}
        </div>
      </div>
      <div className="space-y-3">
        {filtered.map((s:any, i:number) => (
          <div key={i} className="rounded-xl border border-border bg-gradient-to-br from-slate-900/80 to-black/90 p-4">
            ...
            <div className="text-sm font-semibold text-foreground mb-1">{s.metric}</div>
