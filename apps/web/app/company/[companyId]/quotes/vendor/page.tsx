...
export default async function CompanyVendorQuotesPage({ params, searchParams }: Props) {
    ...
    <AppLayout>
      <div className="space-y-4 py-4">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-semibold">Vendor Quotes</h1>
            <p className="text-sm text-muted-foreground">
              Create and track quotations received from vendors.
            </p>
          </div>
                    ...
                    <tr key={quote.id} className="border-b last:border-0">
                      <td className="px-3 py-3">
                        ...
                        <div className="text-xs text-muted-foreground">
                          Estimate {quote.estimateId ? quote.estimateId.slice(0, 8) : "—"}
                        </div>
                      </td>
                      <td className="px-3 py-3 text-sm">
                        {quote.meta?.vendorName ?? quote.vendorId?.slice(0, 8) ?? "—"}
                      </td>
                      <td className="px-3 py-3">
                      ...
                      <td className="px-3 py-3 text-right">
                        <a
                          href={`/company/${companyId}/quotes/vendor/${quote.id}`}
