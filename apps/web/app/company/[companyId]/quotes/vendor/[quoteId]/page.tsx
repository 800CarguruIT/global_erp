import { AppLayout, VendorQuoteDetailMain } from "@repo/ui";

type Props =
  | { params: { companyId: string; quoteId: string } }
  | { params: Promise<{ companyId: string; quoteId: string }> };

export default async function CompanyVendorQuoteDetailPage({ params }: Props) {
  ...
  return (
    <AppLayout>
      <VendorQuoteDetailMain companyId={companyId} quoteId={quoteId} />
    </AppLayout>
  );
}
