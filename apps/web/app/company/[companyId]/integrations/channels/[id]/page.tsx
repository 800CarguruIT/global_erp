...
export default function CompanyChannelsEditPage({ params }: Params) {
    ...
    async function load() {
      ...
      try {
        const res = await fetch(
          `/api/channels/integrations/${integrationId}?scope=company&companyId=${companyId}`
        );
        if (!res.ok) {
  ...
  return (
    <AppLayout>
      <div className="space-y-6 py-4">
        <h1 className="text-xl sm:text-2xl font-semibold">Edit Company Channel Integration</h1>
