...
export function VendorsAiBox({ companyId }: { companyId: string }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      setError(null);
      try {
        const res = await fetch(`/api/company/${companyId}/vendors/ai-summary`);
    ...
    <div className="space-y-3">
      {totals && (
        <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3 text-xs sm:text-sm">
          <Kpi label="Vendors" value={totals.vendors} />
          <Kpi label="Users" value={totals.users} />
          <Kpi label="Quotes" value={totals.quotes} />
          <Kpi label="PO/LPO" value={totals.pos} />
          <Kpi label="Delivered items" value={totals.delivered} />
          <Kpi label="Returned items" value={totals.returned} />
        </div>
      )}
