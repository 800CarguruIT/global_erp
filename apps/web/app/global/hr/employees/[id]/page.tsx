...
export default function GlobalEmployeeEditPage({
  params,
}: {
  params: { id: string };
}) {
  ...
  useEffect(() => {
    async function load() {
      setError(null);
      try {
        const res = await fetch(`/api/hr/employees/${params.id}?scope=global`);
  ...
  return (
    <AppLayout>
      <div className="space-y-4 py-4">
        ...
        {initial ? (
          <EmployeeForm mode="edit" scope={{ type: "global" }} initialValues={initial} />
        ) : (
