...
function normalizeSuggestions(value: unknown): SmartFaultSuggestion[] {
  ...
  return value
    .map((item, index) => {
      ...
      const fallbackId =
        label.toLowerCase().replace(/[^a-z0-9]+/g, "-") || `s${index + 1}`;
...
export async function POST(req: NextRequest, { params }: Params) {
  try {
    ...
    const filteredCatalogCandidates = catalogCandidates.filter((item) => {
      ...
      if (rawLabel.includes(partLower)) return false;
      if (codeLower && rawNumber.includes(codeLower)) return false;
      return true;
    });

    const allowed = await canUseAi(
      "ai.workshop.inspection.related_suggestions" as any,
      { companyId },
    ).catch(() => true);
...
- Severity: ${status || "N/A"}
- VIN: ${vin || "N/A"}
- VIN catalog candidates: ${JSON.stringify(
...
`;
    ...
    console.error(
      "POST /api/mobile/company/[companyId]/workshop/inspections/ai-related-suggestions error:",
      error,
    );
