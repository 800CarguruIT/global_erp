  ...
  try {
    ...
    if (parsed.data.action === "car_in") {
      ...
      await appendLeadEvent({
        ...
        leadId: lead.id,
        eventType: "car_in",
        eventPayload: { checkinAt, source: "customer_car_select" },
      });
    ...
    if (appointmentType === "recovery") {
      ...
      await appendLeadEvent({
        ...
        eventPayload: { appointmentAt, appointmentType, source: "customer_car_select" },
      });
      return NextResponse.json({ data: lead }, { status: 201 });
    }
