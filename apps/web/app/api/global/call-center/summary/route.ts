import { NextRequest, NextResponse } from "next/server";
import { CallCenter, Dialer } from "@repo/ai-core/server";

function todayRange() {
  const to = new Date();
  const from = new Date();
  from.setHours(0, 0, 0, 0);
  return { from, to };
}

export async function GET(req: NextRequest) {
  try {
    const { from, to } = todayRange();

    const dashboard = await CallCenter.getDashboardData({
      scope: "global",
      from,
      to,
    });

    const recentCalls = await CallCenter.listRecentCalls({
      scope: "global",
      limit: 20,
    });

    const dialers = await Dialer.listGlobalDialers();

    return NextResponse.json({
      stats: {
        totalCallsToday: dashboard.totals.totalCalls,
        answeredToday: dashboard.totals.completedCalls,
        missedToday: dashboard.totals.failedCalls,
        outboundToday:
          dashboard.byDirection.find((d) => d.direction === "outbound")?.count ?? 0,
      },
      recentCalls: recentCalls.map((c: any) => ({
        id: c.id,
        companyName: null,
        from: c.fromNumber,
        to: c.toNumber,
        direction: c.direction,
        status: c.status,
        startedAt: c.createdAt,
        durationSeconds: c.durationSeconds ?? null,
      })),
      activeDialers: dialers.map((d) => ({
        id: d.id,
        label: d.label,
        provider: d.provider,
        level: d.is_global ? "global" : "company",
        companyName: null,
      })),
    });
  } catch (err) {
    console.error("call-center summary error", err);
    return NextResponse.json(
      {
        stats: { totalCallsToday: 0, answeredToday: 0, missedToday: 0, outboundToday: 0 },
        recentCalls: [],
        activeDialers: [],
      },
      { status: 200 }
    );
  }
}
