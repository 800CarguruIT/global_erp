...
export async function GET(req: NextRequest, { params }: Params) {
  ...
  try {
    const allowed = await canUseAi("ai.vendorQuotes.summary" as any, { companyId }).catch(() => true);
    ...
    const submitted = byStatus["submitted"]?.count ?? 0;
    const drafts = byStatus["draft"]?.count ?? 0;

    const client = getOpenAIClient();
    const prompt = `
You are an AI vendor-quote analyst. Provide 2-4 concise actions and one appreciation based on these metrics:
- Total vendor quotes: ${totalQuotes}
- Draft: ${drafts}
- Submitted: ${submitted}
- Approved: ${approved}
- Rejected: ${rejected}
...
`;
    ...
    console.error("GET /api/company/[companyId]/quotes/vendor/ai-summary error", err);
