...
function mapToForm(company: any, contacts: any[]) {
  ...
  return {
    ...
    // Extra flags used by other parts of the UI (e.g., punch order VAT defaults)
    vat_enabled: company.has_vat_tax ?? false,
...
function mapToUpdatePayload(body: any) {
  return {
    ...
    vat_rate: body.taxSettings?.vatRate ?? body.vat_rate ?? body.vatRate ?? null,
...
async function resolveCompanyId(params: any): Promise<string | null> {
...
export async function GET(
  _req: NextRequest,
  { params }: { params: { companyId: string } | Promise<{ companyId: string }> }
) {
  try {
    const companyId = await resolveCompanyId(params);
    ...
    return NextResponse.json({ data: mapToForm(company, contacts ?? []) });
  } catch (error) {
...
export async function PATCH(
  ...
  try {
    ...
    const contacts = Array.isArray(body.contacts)
      ...
      : undefined;
    const updated = await updateCompanyProfile({ companyId, data: patch, contacts });
    return NextResponse.json({ data: mapToForm(updated, contacts ?? []) });
  } catch (error) {
    console.error("PATCH /api/company/[companyId]/profile error", error);
